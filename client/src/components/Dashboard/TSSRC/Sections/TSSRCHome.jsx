...
export default function TSSRCHome({ theme }) {
    ...
    return (
        <div className="space-y-6">
            {/* Welcome Header */}
            <div>
                <h2 className={`text-2xl font-bold ${textColor}`}>TSSRC Dashboard</h2>
                <p className={`${subTextColor} text-sm mt-1`}>Technical Screening and Standing Research Committee - Proposal Review Panel</p>
            </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div className={`p-4 rounded-xl ${isDarkest ? 'bg-neutral-800' : isDark ? 'bg-slate-700/50' : 'bg-slate-50'}`}>
                        ...
                        <ul className={`text-sm ${subTextColor} space-y-1`}>
                            <li>• Review proposals forwarded by CMPDI</li>
                            <li>• Provide technical assessment</li>
                            <li>• Recommend proposals for SSRC review</li>
                            <li>• Request clarifications from investigators</li>
                        </ul>
                    </div>
                    <div className={`p-4 rounded-xl ${isDarkest ? 'bg-neutral-800' : isDark ? 'bg-slate-700/50' : 'bg-slate-50'}`}>
                        <h4 className={`font-bold ${textColor} mb-2`}>Workflow</h4>
                        <ul className={`text-sm ${subTextColor} space-y-1`}>
                            <li>1. Receive proposals from CMPDI</li>
                            <li>2. Conduct technical review</li>
                            <li>3. Add comments and recommendations</li>
                            <li>4. Forward to SSRC or send back</li>
                        </ul>
                    </div>
                </div>
            </div>
