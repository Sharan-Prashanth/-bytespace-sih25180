...
export default function SSRCProposalsSection({ theme }) {
    ...
    // Categorize proposals for SSRC view
    // Active: TSSRC_ACCEPTED (incoming), SSRC_REVIEW
    const activeProposals = proposals.filter(p => 
        ['TSSRC_ACCEPTED', 'SSRC_REVIEW'].includes(p.status)
    );
    
    // Final Approved (Funded Projects): SSRC_ACCEPTED
    const approvedProposals = proposals.filter(p => p.status === 'SSRC_ACCEPTED');
    
    // Rejected: SSRC_REJECTED
    const rejectedProposals = proposals.filter(p => p.status === 'SSRC_REJECTED');
        ...
        switch (viewSection) {
            case 'active':
                return [
                    ...
                    { value: 'TSSRC_ACCEPTED', label: 'Incoming from TSSRC' },
                    { value: 'SSRC_REVIEW', label: 'Under Review' }
                ];
            case 'approved':
                return [
                    { value: 'all', label: 'All Status' },
                    { value: 'SSRC_ACCEPTED', label: 'Approved for Funding' }
                ];
            case 'rejected':
                return [
                    { value: 'all', label: 'All Status' },
    ...
    const renderProposalRow = (proposal) => {
        ...
        return (
            <tr key={proposal._id} className={`group transition-colors ${hoverBg}`}>
                ...
                <td className="p-3 px-4">
                    {isApprovedSection ? (
                        <span className={`inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-xs font-bold border ${isDark ? 'bg-teal-900/30 text-teal-400 border-teal-500/30' : 'bg-teal-50 text-teal-700 border-teal-200'}`}>
                            <Rocket size={12} />
    ...
    const renderProposalCard = (proposal) => {
        const statusConfig = STATUS_CONFIG[proposal.status];
        const isRejected = proposal.status === 'SSRC_REJECTED';
            ...
            <div key={proposal._id} className={`${cardBg} p-4 rounded-xl shadow-sm border flex flex-col transition-all hover:shadow-md ${hoverBg}`}>
                <div className="flex justify-between items-start mb-2">
                    <div className={`w-10 h-10 rounded-xl flex items-center justify-center ${
                        ...
                        {isRejected ? <XCircle size={20} /> : isApproved ? <Rocket size={20} /> : <FileText size={20} />}
                    </div>
                    {isApproved ? (
                        <span className={`inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-[10px] font-bold border ${isDark ? 'bg-teal-900/30 text-teal-400 border-teal-500/30' : 'bg-teal-50 text-teal-700 border-teal-200'}`}>
                            <Rocket size={10} />
                            Funded
                        </span>
                    ) : (
                ...
                {isApproved && (
                    <div className={`text-[10px] mb-2 p-2 rounded-lg ${isDark ? 'bg-teal-900/20 border border-teal-500/20' : 'bg-teal-50 border border-teal-200'}`}>
                        <p className={`font-bold ${isDark ? 'text-teal-400' : 'text-teal-700'}`}>
                            <CheckCircle size={12} className="inline mr-1" />
                            Approved for S&T Funding
                        </p>
                        <p className={`${isDark ? 'text-teal-300' : 'text-teal-600'} mt-1`}>
                            Project approved by SSRC. Contact investigator to initiate project.
                        </p>
                    </div>
                )}
    ...
    return (
        <div className="space-y-4">
            ...
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
                <div>
                    <h2 className={`text-xl font-bold ${textColor}`}>SSRC Proposals</h2>
                    <p className={`${subTextColor} text-sm`}>Final review and approval of research proposals.</p>
                </div>
            </div>
            ...
            <div className={`${cardBg} p-3 rounded-xl shadow-sm border flex flex-col sm:flex-row items-center gap-3`}>
                ...
                <div className={`flex items-center p-1 rounded-xl border flex-wrap ${isDarkest ? 'bg-neutral-950 border-neutral-800' : isDark ? 'bg-slate-900 border-slate-700' : 'bg-slate-50 border-slate-200'}`}>
                    ...
                    {approvedProposals.length > 0 && (
                        ...
                        >
                            <Rocket size={14} />
                            Funded Projects ({approvedProposals.length})
                        </button>
                    )}
