      ...
      >
        <div className="flex items-center gap-3">
          ...
          <div className="relative" onClick={(e) => e.stopPropagation()}>
            ...
            {/* Tooltip */}
            {showInfoTooltip && (
              <div className={`absolute left-0 top-full mt-2 z-50 w-80 p-4 rounded-lg shadow-xl border ${isDark ? 'bg-slate-700 border-slate-600' : 'bg-white border-black/10'}`}>
                ...
                <p className={`text-xs ${textColor} mb-3`}>
                  This section is for <strong>general proposal comments</strong> only. These are high-level discussions about the proposal as a whole.
                </p>
                <ul className={`text-xs ${textColor} space-y-2`}>
                  <li className="flex items-start gap-2">
                    ...
                    <span>Use for overall feedback, questions, or suggestions about the proposal.</span>
                  </li>
                  <li className="flex items-start gap-2">
                    <svg className="w-4 h-4 mt-0.5 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                      <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15.232 5.232l3.536 3.536m-2.036-5.036a2.5 2.5 0 113.536 3.536L6.5 21.036H3v-3.572L16.732 3.732z" />
                    </svg>
                    <span><strong>For comments on specific text:</strong> Select text in the editor and use inline comments instead.</span>
                  </li>
                  <li className="flex items-start gap-2">
                    <svg className="w-4 h-4 mt-0.5 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                      <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M5 13l4 4L19 7" />
                    </svg>
                    <span>Only the comment author can mark it as resolved.</span>
                  </li>
                </ul>
                <div className={`mt-3 pt-3 border-t ${borderColor} text-xs ${isDark ? 'text-slate-400' : 'text-black/60'}`}>
                  Inline comments in the editor are stored separately
                </div>
              </div>
            )}
          </div>
              ...
              <div className="flex items-center justify-between mb-2">
                <div className="flex items-center gap-2">
                  <div className={`w-7 h-7 ${avatarBg} rounded-full flex items-center justify-center text-xs font-semibold ${textColor}`}>
                    {comment.author?.fullName?.charAt(0) || comment.author?.name?.charAt(0) || 'U'}
                  </div>
                    ...
                    <div key={reply._id || idx} className="text-sm">
                      <div className="flex items-center gap-2 mb-1">
                        <span className={`font-semibold ${textColor}`}>
                          {reply.author?.fullName || reply.author?.name || 'Unknown User'}
                        </span>
                ...
                <div className="flex items-center gap-2">
                  ...
                  {isCommentAuthor(comment) && (
                    <button
                      ...
                      className={`px-2 py-1 text-xs font-medium ${activeBtnBg} rounded transition-colors flex items-center gap-1`}
