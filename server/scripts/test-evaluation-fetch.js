...
/**
 * Test script to fetch and display evaluation report data
 * Usage: node scripts/test-evaluation-fetch.js <proposalId>
 */

async function fetchEvaluationReport(proposalId) {
  try {
    // Connect to MongoDB
    ...
    console.log('='.repeat(80));
    console.log(`Total evaluation reports: ${evaluationReports.length}\n`);

    // Get latest evaluation report
    const latestReport = evaluationReports[evaluationReports.length - 1];

    console.log('='.repeat(80));
    console.log('LATEST EVALUATION REPORT');
    console.log('='.repeat(80));
    ...
    console.log('='.repeat(80));
    console.log('EVALUATION REPORT DATA (JSON)');
    console.log('='.repeat(80));
...
if (!proposalId) {
  console.error('Usage: node scripts/test-evaluation-fetch.js <proposalId>');
  console.error('Example: node scripts/test-evaluation-fetch.js 507f1f77bcf86cd799439011');
  console.error('Example: node scripts/test-evaluation-fetch.js PROP-2024-001');
  process.exit(1);
}

console.log(`\nFetching evaluation report for proposal: ${proposalId}\n`);
fetchEvaluationReport(proposalId);
